...
int main(int argc, char** argv) {
    ...
    pyrMeanShiftFiltering(src, shifted, 23, 51);  // 边缘保留滤波
    // imshow("shifted", shifted);

    cvtColor(shifted, gray, COLOR_BGR2GRAY);                       // 转化为灰度图
    ...
    // binary
    threshold(dist, dist, 0.2, 0.7, THRESH_BINARY);  // 限定阈值
    imshow("distance binary", dist);

    // markers
    Mat dist_m;
    dist.convertTo(dist_m, CV_8U);  // 转化为单通道图像

    vector<vector<Point>> contours;  // 定义数据存放轮廓
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        colors.push_back(Vec3b((uchar)b, (uchar)g, (uchar)g));
    }
